...
(function ($, document) {
  ...
  var Cloner = {
    init: function (options, elem) {
      ...
      self.$closeButton = self.$container.closestChild(
    ...
    toggle: function (options, self) {
       ...
       */
      $clone
        .removeClass(self.options.sourceName)
        .addClass(self.options.cloneName);
       ...
       */
      if (
        self.options.limitCloneNumbers &&
        self.$clone.hasClass(self.options.clonableCloneNumberDecrement) &&
        self.cloneNumber === 1
      ) {
        return true;
      }
    ...
    increment: function ($clone, index, self) {
       ...
       * @type Object
       */
      var incrementables = $clone
        .find('[class*="' + self.options.incrementName + '"]')
        .filter(function (i, e) {
       ...
       * @type int
       */
      var _i = $clone[0].hasAttribute("clone-number")
        ? $clone.data("clone-number")
        : index;

      r.debug(
        "[Cloner]: start increment | Clone number: " + _i + " | Index: " + index
      );
          ...
          if (reg.test(classes[i])) {
            ...
            switch (attr) {
              case "value":
                ...
                var new_val = old_val.replace(/-?(\d+)(?!.*\d)/g, function (n) {
                  return ++n;
                });
                $(this).val(new_val);
                break;
    ...
    decrement: function ($clone, index, self) {
       ...
       * @type Object
       */
      var decrementables = $clone
        .find('[class*="' + self.options.decrementName + '"]')
        .filter(function (i, e) {
          return $(e).closest(self.options.clonable).get(0) == $clone.get(0);
        });
  ...
  $.fn.cloner = function (options) {
    ...
    return this.each(function () {
      ...
      $(addButton).on("click", function (e) {
        ...
        self.$clonables = $(this)
          .closest(self.options.clonableContainer)
          .closestChild(self.options.clonable);
      ...
      cloner.remove(function (self) {
        $(document).on("click", self.options.closeButton, function (e) {
          self.options.recount(this.parentElement);
  ...
  $.fn.cloner.options = {
    ...
    recount: function (element) {
      let nextElement = $(element).next();
      while (nextElement.hasClass("clonable-clone")) {
        const childrens = nextElement.children();
        const label = childrens[0];
        let attr = label.htmlFor.split("_");
        label.htmlFor = `${attr[0]}_${attr[1] - 1}`;
        label.children[0].innerText = label.children[0].innerText - 1;
        const input = childrens[1];
        attr = input.id.split("_");
        input.id = `${attr[0]}_${attr[1] - 1}`;
        attr = input.name.split("[");
        input.name = `${attr[0]}[${attr[1][0] - 1}]`;
